# Hi 👋, I'm Dubakula Ramya

### Software Engineer | Full-Stack Developer | AI Enthusiast

🎓 Integrated MCA (IMCA) Student (2022–2026) at Parul University

💻 Passionate about building scalable web applications, backend systems, and AI-powered solutions.

---

## 🚀 About Me

* 🌱 Currently learning **Software Engineering, System Design, and Cloud Technologies**
* 💼 Interested in **Software Engineering, Full-Stack Development, Backend Development, and AI**
* 💡 Strong foundation in **Data Structures & Algorithms, Object-Oriented Programming, REST APIs, and Software Development**
* 📫 Reach me at **[dubakularamya6@gmail.com](mailto:dubakularamya6@gmail.com)**
* 📍 Hyderabad, Telangana, India

---

## 💻 Tech Stack

### Languages

* Java
* Python
* JavaScript
* SQL

### Frontend

* React.js
* HTML5
* CSS3
* Tailwind CSS

### Backend

* Flask
* FastAPI
* REST APIs

### Databases

* MySQL
* PostgreSQL
* MongoDB

### Tools

* Git
* GitHub
* VS Code
* Postman

---

## 🚀 Featured Projects

### 🤖 NL2SQL Engine

AI-powered Natural Language to SQL Query Engine built using Python, Claude API, SQL, and Prompt Engineering.

### 🛒 Full-Stack E-commerce Marketplace

Developed a scalable marketplace using React, TypeScript, Tailwind CSS, FastAPI, and MongoDB.

### 🛡️ Cloud-Based Fraud Detection

Machine Learning-based fraud detection system for real-time transaction monitoring and anomaly detection.


### 🌤️ Weather Forecast App

Weather application using OpenWeather API and JavaScript.

---



## 🤝 Connect With Me

* 💼 LinkedIn: https://www.linkedin.com/in/dubakula-ramya-1ab0b4294
* 💻 GitHub: https://github.com/ramyadubakula03

⭐ Thanks for visiting my profile!
